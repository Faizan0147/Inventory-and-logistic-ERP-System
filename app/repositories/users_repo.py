...
async def create_user(
    ...
    row = await conn.fetchrow(
        """
        INSERT INTO users (user_id, name, email, password_hash, phone_number, role, supplier_id, is_active, created_by, updated_by)
        VALUES ($1, $2, $3, $4, $5, $6, $7, TRUE, $8, $8)
        RETURNING user_id, name, email, phone_number, role, supplier_id, is_active,
                  created_at, updated_at, created_by, updated_by
        """,
        str(uuid4()), data.name, data.email, password_hash, data.phone_number,
        data.role, data.supplier_id, created_by,
    )
...
async def get_user_by_email(conn: asyncpg.Connection, email: str) -> Optional[dict]:
    row = await conn.fetchrow(
        """
        SELECT user_id, name, email, password_hash, phone_number, role, supplier_id, is_active
        FROM users
...
async def get_user(conn: asyncpg.Connection, user_id: str) -> Optional[UserRead]:
    row = await conn.fetchrow(
        """
        SELECT user_id, name, email, phone_number, role, supplier_id, is_active,
               created_at, updated_at, created_by, updated_by
        FROM users
...
async def update_user(
    ...
    row = await conn.fetchrow(
        ...
        WHERE user_id = $9 AND deleted = FALSE
        RETURNING user_id, name, email, phone_number, role, supplier_id, is_active,
                  created_at, updated_at, created_by, updated_by
        """,
        data.name, data.email, password_hash, data.phone_number,
        data.role, data.supplier_id, data.is_active, updated_by, user_id,
    )
