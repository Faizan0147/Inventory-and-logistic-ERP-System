...
async def get_inventory(
    conn: asyncpg.Connection, inventory_id: str
) -> InventoryRead:
    inventory = await inventory_repo.get_inventory(conn, inventory_id)
    if not inventory:
...
async def list_inventory(
    ...
    return await inventory_repo.list_inventory(conn, offset, limit)
...
async def update_inventory(
    ...
    inventory = await inventory_repo.update_inventory(
        conn, inventory_id, body, updated_by=current_user["user_id"]
    )
    if not inventory:
        raise HTTPException(status_code=404, detail="Inventory not found")
    return inventory
...
async def delete_inventory(
    ...
    deleted = await inventory_repo.delete_inventory(
        conn, inventory_id, deleted_by=current_user["user_id"]
    )
    if not deleted:
        raise HTTPException(status_code=404, detail="Inventory not found")