...
async def list_warehouse(conn: Conn, current_user: CurrentUser, offset: int = 0, limit: int = 100):
    return await warehouse_controller.list_warehouses(conn, offset, limit)


@router.get("/{warehouse_id}", response_model=WarehouseRead)
async def get_warehouse(warehouse_id: str, conn: Conn, current_user: CurrentUser):
    return await warehouse_controller.get_warehouse(conn, warehouse_id)
...
async def delete_warehouse(warehouse_id: str, conn: Conn, current_user: CurrentUser):
    await warehouse_controller.delete_warehouse(conn, warehouse_id, current_user)